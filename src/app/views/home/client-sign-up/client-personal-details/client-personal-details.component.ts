...
export class ClientPersonalDetailsComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.rForm.valueChanges.subscribe(data => {
      ...
      const cellPhone = data.CellphoneNumber;
      ...
      if (
        this.allUsers.filter(x => x.CellphoneNumber == cellPhone).length > 0
      ) {
        this.cellUsed =
  ...
  createClientAccount(data) {
    ...
    this.accountService.addClient(data).subscribe(response => {
      ...
      if (response.ClientId) {
        ...
        localStorage.setItem(LAST_INSERT_EMAIL,  data.Email);

        // #todo  - update state
        this.messageService.add({
          life: 7000,
          severity: 'success',
          summary: 'Welcome to Citgo Africa Oil famaily!',
  ...
  verifyAcc(name, email, link) {
    ...
    this.emailService.sendVerifyAcc(data).subscribe(r => {
      ...
      this.progress = `To ensure that your email account is 
      valid, we have sent you an email to  ${email} to  verify your account,  please check your mailbox`;
    });
  }
